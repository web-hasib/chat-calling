...
export function useChat(
  ...
  const confirmDeleteMessage = (targetMsgId: string) => {
    ...
    socket.emit('delete-message', { messageId: targetMsgId, conversationId: activeConvo.id });
  ...
  useEffect(() => {
    ...
    socket.on('new-message-notification', (msg: any) => {
      setConversations((prevConvos) => {
        ...
        const updated = [...prevConvos];
        const convo = { ...updated[index], messages: [msg] };
        updated.splice(index, 1);
        return [convo, ...updated];
      });
  ...
  useEffect(() => {
    if (!socket || !activeConvo) { setIsRecipientTyping(false); return; }
    const eventName = `typing-${activeConvo.id}`;
    socket.on(eventName, (data: { userId: string; isTyping: boolean }) => {
      if (data.userId !== user?.id) setIsRecipientTyping(data.isTyping);
    });
    return () => { socket.off(eventName); };
  }, [socket, activeConvo, user, setIsRecipientTyping]);
